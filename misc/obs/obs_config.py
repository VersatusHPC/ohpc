...
class ohpc_obs_tool(object):
    ...
    def parseConfig(self, configFile=None, service_file=None):
        ...
        if os.path.isfile(configFile):
            ...
            try:
                ...
                ERROR(
                    "\nERROR: Duplicate section detected in configfile: %s" %
                    configFile
                )
            except Exception:
                ERROR(
                    "ERROR; Unable to parse runtime config file: %s" %
                    configFile
                )
            ...
            logging.info("--> (global) %i package groups defined:" %
                         len(groups))
            ...
            for group in groups:
                try:
                    components = ast.literal_eval(
                        self.buildConfig.get("groups", group))
            ...
            for group in groups:
                logging.info("    --> %-20s: %2i components included" %
    ...
    def isStandalone(self, package):
        ...
        logging.debug("\n[%s] - %s: compiler_dep = %s" %
                      (fname, package, compiler_dep))
        logging.debug("[%s] - %s: mpi_dep      = %s" %
                      (fname, package, mpi_dep))
    ...
    def checkPackageGroup(self, package):
        ...
        for group in self.groups:
            if package in self.groups[group]:
                logging.debug("[%s] %s belongs to group %s" %
                              (fname, package, group))
                return group
        if not found:
            ERROR(
    ...
    def addPackage(
        ...
        if os.path.isfile(self.serviceFile):
            ...
            if os.path.isfile("%s/_service.%s" % (self.overrides, package)):
                logging.warning(
                    " " * pad +
                    "--> package-specific _service file provided for %s" %
                    package
                )
        ...
        if self.disableBuild(package, 'aarch64'):
            logging.warning(
                " " * pad +
                "--> disabling aarch64 build per pattern match request")
        ...
        if self.disableBuild(package, 'x86_64'):
            logging.warning(
                " " * pad +
                "--> disabling x86_64 build per pattern match request")
...
def main():
    ...
    parser.add_argument(
        "--package",
        help="check OBS config for provided package only",
        type=str)
    parser.add_argument(
        "--service-file",
        help=(
    ...
    if not args.dryrun:
        logging.info(
            "--no-dryrun command line arg requested: will execute commands\n")
        obs.overrideDryRun()

    # override lock option if requested
    if not args.lock:
        logging.info(
            "--no-lock command line arg requested: will not lock new builds\n")
        obs.overrideLock()
    ...
    for package in components['comp_dep']:
        ...
        if compilers != Defcompilers:
            ...
            logging.warning(
                " " *
                pad +
                "--> override of default compiler families requested for %s" %
                package)
