...
if [[ -n "${DRY_RUN}" && -d "${STAGING_DIR}" ]]; then
    ...
    stage_container_rpms "${EL10_CONTAINER}" "${CONTAINER_RPMBUILD}/RPMS/noarch"  "${STAGING_DIR}/EL_10/noarch"  "meta-packages*.rpm" "*.ci.ohpc*"
    ...
    stage_container_rpms "${OE_CONTAINER}" "${CONTAINER_RPMBUILD}/RPMS/noarch"  "${STAGING_DIR}/openEuler_24.03/noarch"  "meta-packages*.rpm" "*.ci.ohpc*"
