\newcolumntype{C}[1]{>{\centering}p{#1}}
\newcolumntype{L}[1]{>{\raggedleft}p{#1}}
\small
\begin{tabularx}{\textwidth}{L{\firstColWidth{}}|C{\secondColWidth{}}|X}
\toprule
{\bf RPM Package Name} & {\bf Version} & {\bf Info/URL}  \\
\midrule

pdtoolkit-gnu15-ohpc &
\multirow{2}{*}{3.25.1} &
\multirow{2}{\linewidth}{Program Database Toolkit (gnu15, OpenHPC). \newline {\color{logoblue} \url{http://www.cs.uoregon.edu/Research/pdt}}}\\
pdtoolkit-intel-ohpc &
& \\
\hline
% <-- end entry for pdtoolkit

% <-- begin entry for scalasca
scalasca-gnu15-impi-ohpc &
\multirow{8}{*}{2.6.2} &
\multirow{8}{\linewidth}{Toolset for performance analysis of large-scale parallel applications (gnu15 + impi, OpenHPC). \newline {\color{logoblue} \url{http://www.scalasca.org}}}\\
scalasca-gnu15-mpich-ohpc &
& \\
scalasca-gnu15-mvapich2-ohpc &
& \\
scalasca-gnu15-openmpi5-ohpc &
& \\
scalasca-intel-impi-ohpc &
& \\
scalasca-intel-mpich-ohpc &
& \\
scalasca-intel-mvapich2-ohpc &
& \\
scalasca-intel-openmpi5-ohpc &
& \\
\hline
% <-- end entry for scalasca

% <-- begin entry for scorep
scorep-gnu15-impi-ohpc &
\multirow{8}{*}{9.4} &
\multirow{8}{\linewidth}{Scalable Performance Measurement Infrastructure for Parallel Codes (gnu15 + impi, OpenHPC). \newline {\color{logoblue} \url{http://www.vi-hps.org/projects/score-p}}}\\
scorep-gnu15-mpich-ohpc &
& \\
scorep-gnu15-mvapich2-ohpc &
& \\
scorep-gnu15-openmpi5-ohpc &
& \\
scorep-intel-impi-ohpc &
& \\
scorep-intel-mpich-ohpc &
& \\
scorep-intel-mvapich2-ohpc &
& \\
scorep-intel-openmpi5-ohpc &
& \\
\hline
% <-- end entry for scorep

% <-- begin entry for tau
tau-gnu15-impi-ohpc &
\multirow{8}{*}{2.35.1} &
\multirow{8}{\linewidth}{Tuning and Analysis Utilities profiling package (gnu15 + impi, OpenHPC). \newline {\color{logoblue} \url{http://www.cs.uoregon.edu/research/tau/home.php}}}\\
tau-gnu15-mpich-ohpc &
& \\
tau-gnu15-mvapich2-ohpc &
& \\
tau-gnu15-openmpi5-ohpc &
& \\
tau-intel-impi-ohpc &
& \\
tau-intel-mpich-ohpc &
& \\
tau-intel-mvapich2-ohpc &
& \\
tau-intel-openmpi5-ohpc &
& \\
\hline
% <-- end entry for tau

\bottomrule
\end{tabularx}
