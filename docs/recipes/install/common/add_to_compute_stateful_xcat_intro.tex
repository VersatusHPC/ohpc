...
[sms](*\#*) psh compute dnf config-manager --set-enabled crb
\end{lstlisting}
% end_ohpc_run

\noindent Next, the nodes are configured to use the local \OHPC{} repository
setup on the SMS in \S\ref{sec:enable_repo}
...
large ($>$10 GB) containing thousands of packages, while only a few
are required for packages installed in this guide. Therefore,
...
[sms](*\#*) (*\install*) dnf-plugins-core createrepo_c
...
[sms](*\#*) createrepo_c $epel_repo_dir
