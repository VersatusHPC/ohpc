...
| 2026-04-18 | CUDA repository package | `cuda-repo-ohpc` installs Ubuntu NVIDIA CUDA and NVIDIA HPC SDK APT source files plus scoped keyrings; the Ubuntu GPU recipe installs `cuda-drivers` instead of the RPM-only `nvidia-driver-cuda` name. | NVIDIA publishes different repository layouts and package names for Debian/Ubuntu than for EL/openEuler, while OpenHPC expects one convenience package to enable NVIDIA GPU repositories. | Upstream OpenHPC adds native Debian/Ubuntu CUDA repository packaging or NVIDIA changes the Ubuntu package naming/layout. |
